...
class lowpass_filter:
    ...
    def initUI(self, master):
        Label(master, text="Original Gray Image", anchor="w").pack(fill='both')
        self.frame1.pack()
        Label(master, text="Average filter", anchor="w").pack(fill='both')
        self.frame2.pack()
        Label(master, text="Nonlinear: median filter", anchor="w").pack(fill=X)
        self.frame3.pack()
        ...
        if len(lowpass_filter.path) > 0:
            ...
            hbtn = Button(self.frame4, text="OPEN IMAGE", command=lambda: self.button_click(master))
...
def main():
    ...
    filter_class = lowpass_filter(root)
